...
export default function AdminTracerStudyScreen() {
  ...
  const fetchVerifications = async () => {
    setLoading(true);
    try {
      const data = await ijazahValidationService.loadAdminVerifications();
  ...
  const handleApproveAll = async () => {
    ...
    try {
      const pendings = verifications.filter((v) => v.status === "pending_verification");
      for (const p of pendings) {
        await ijazahValidationService.approveRequirement(p.id);
      }
      await fetchVerifications();
      Alert.alert("Sukses", "Semua berkas masuk telah disetujui.");
    } catch (error) {
      Alert.alert("Gagal", "Gagal menyetujui semua berkas.");
    }
  };
  ...
  const handleRejectAll = async () => {
    ...
    try {
      const pendings = verifications.filter((v) => v.status === "pending_verification");
      for (const p of pendings) {
        await ijazahValidationService.rejectRequirement(p.id);
      }
      await fetchVerifications();
      Alert.alert("Sukses", "Semua berkas masuk telah ditolak.");
    } catch (error) {
      Alert.alert("Gagal", "Gagal menolak semua berkas.");
    }
  };
                ...
                <View className="border-x border-b border-gray-200 rounded-b-2xl overflow-hidden bg-white">
                  {loading ? (
                    <View className="py-16 items-center justify-center w-full">
                      ...
                      <Text className="text-gray-500 font-semibold text-sm mt-3">Memuat data pengajuan...</Text>
                    </View>
                  ) : verifications.length > 0 ? (
                    verifications.map((v, index) => {
                      const isCompleted = v.status === "verified";
                      const isPendingVerification = v.status === "pending_verification";
                      const isRejected = v.status === "rejected";
                      const isUploading = v.status === "uploading";
                      const isPendingUpload = v.status === "pending_upload";
                      
                      const vTitle = getTitleForId(v.requirement_id || v.id);
                      const vDesc = getDescForId(v.requirement_id || v.id);
                      const studentName = v.user?.alumni_profile?.nama || "Alumni Demo";
                      const studentNim = v.user?.alumni_profile?.nim || "1902001";
                      const studentProdi = v.user?.alumni_profile?.prodi || "Informatika";

                      return (
                        <View key={v.id} className={`flex-row items-center py-4 px-4 border-b border-gray-100 ${index % 2 === 1 ? 'bg-slate-50/20' : ''}`}>
                          
                          {/* No */}
                          <View className="w-[50px]">
                            <Text className="text-navy font-bold text-sm">{index + 1}</Text>
                          </View>
                          
                          {/* Persyaratan & Deskripsi */}
                          <View className="flex-1 pr-6 gap-0.5">
                            <Text className="font-bold text-navy text-sm md:text-base">{vTitle}</Text>
                            <Text className="text-gray-500 text-[11px] leading-relaxed">{vDesc}</Text>
                            <View className="flex-row items-center gap-1.5 mt-1.5 bg-slate-100 px-2 py-0.5 rounded-lg self-start">
                              <Ionicons name="person-outline" size={10} color="#475569" />
                              <Text className="text-slate-600 font-extrabold text-[10px] uppercase">
                                {studentName} ({studentNim} - {studentProdi})
                              </Text>
                            </View>
                          </View>
                          ...
                          <View className="w-[200px] pr-4">
                            {v.file_name ? (
                              <View className="flex-row items-center gap-1.5 bg-gray-50 border border-gray-200 px-2.5 py-1.5 rounded-lg self-start">
                                ...
                                <Text className="text-slate-600 text-xs font-semibold max-w-[140px]" numberOfLines={1}>{v.file_name}</Text>
                              </View>
                            ) : (
                          ...
                          <View className="w-[180px] items-center">
                            ...
                            {isCompleted && (v.requirement_id || v.id) > 1 && hasAuthority && (
                              <Pressable 
                                onPress={() => handleReject(v.id)}
                            ...
                            {!isPendingVerification && (!hasAuthority || ((v.requirement_id || v.id) === 1 && isCompleted)) && (
                              <Text className="text-gray-400 text-xs">-</Text>
                            )}
                          </View>
                    ...
                    <View className="py-16 items-center justify-center w-full">
                      ...
                      <Text className="text-gray-500 font-bold text-base mt-4">
                        Tidak ada berkas masuk untuk divalidasi.
                      </Text>
                    </View>
                  )}
                </View>
